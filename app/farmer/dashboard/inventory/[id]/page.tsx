  ...
  const fetchProduct = async () => {
    try {
      ...
      if (res.ok) {
        ...
        let parsedImages: Array<{ id: string; url: string; priority: number; uploadedAt: number }> = []
        if (data.image) {
          if (data.image.startsWith('[')) {
            try {
              const urls = JSON.parse(data.image) as string[]
              parsedImages = urls.map((url, idx) => ({
                id: Math.random().toString(36).substring(2, 9),
                url,
                priority: idx + 1,
                uploadedAt: Date.now() + idx
        ...
        setUpcomingStock(data.upcomingStock || '')
                                ...
                                <div className="flex-1 space-y-1.5 min-w-0">
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <input
                                      ...
                                      className="w-16 rounded-md border border-slate-200 px-2 py-0.5 text-xs text-slate-955 font-bold outline-none focus:border-emerald-505 focus:ring-1 focus:ring-emerald-505/20"
                                    />
                                  </div>
                                </div>
                              </div>
